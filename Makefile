# This Makefile is for the DNRMGWDB extension to perl.
#
...
#     AUTHOR => [q[Sholto Maud, C<< <sholto.maud at gmail.com> >>, Sholto Maud <sholto.maud@gmail.com>]]
#     BUILD_REQUIRES => { ExtUtils::MakeMaker=>q[6.59], Test::More=>q[0], Module::Install=>q[0] }
#     CONFIGURE_REQUIRES => {  }
#     DISTNAME => q[DNRMGWDB]
#     INSTALLDIRS => q[site]
#     LICENSE => q[artistic2]
...
#     NAME => q[DNRMGWDB]
#     NO_META => q[1]
#     PREREQ_PM => { DateTime=>q[0.01], File::Copy=>q[0.01], Env=>q[0.01], ExtUtils::MakeMaker=>q[6.59], Try::Tiny=>q[0.01], FindBin=>q[0.01], JSON=>q[0.01], DBI=>q[0.01], Time::Local=>q[0.01], Test::More=>q[0], Module::Install=>q[0], local::lib=>q[0.01] }
#     TEST_REQUIRES => {  }
#     VERSION => q[0.01]
#     VERSION_FROM => q[lib/DNRMGWDB.pm]
...
DNRMGWDB.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"DNRMGWDB\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  ...
	  lib/DNRMGWDB.pm $(INST_MAN3DIR)\DNRMGWDB.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <AUTHOR>Sholto Maud, C&lt;&lt; &lt;sholto.maud at gmail.com&gt; &gt;&gt;, Sholto Maud &lt;sholto.maud@gmail.com&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,006,0,0\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"DBI::\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"0.01\" NAME=\"DateTime::\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"0.01\" NAME=\"Env::\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"0.01\" NAME=\"File::Copy\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"0.01\" NAME=\"FindBin::\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"JSON::\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Time::Local\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Try::Tiny\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"local::lib\" VERSION=\"0.01\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  ...
	  lib/DNRMGWDB.pm blib\lib\DNRMGWDB.pm 
...
upgradedeps ::
	$(PERL) Makefile.pl --config= --upgradedeps=Test::More,0,Try::Tiny,0.01,Time::Local,0.01,local::lib,0.01,Env,0.01,DateTime,0.01,JSON,0.01,FindBin,0.01,DBI,0.01,File::Copy,0.01

upgradedeps_notest ::
	$(PERL) Makefile.pl --config=notest,1 --upgradedeps=Test::More,0,Try::Tiny,0.01,Time::Local,0.01,local::lib,0.01,Env,0.01,DateTime,0.01,JSON,0.01,FindBin,0.01,DBI,0.01,File::Copy,0.01

listdeps ::
	@$(PERL) -le "print for @ARGV" 

listalldeps ::
	@$(PERL) -le "print for @ARGV" Test::More Try::Tiny Time::Local local::lib Env DateTime JSON FindBin DBI File::Copy
